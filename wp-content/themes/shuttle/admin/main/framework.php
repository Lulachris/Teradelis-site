...
$GLOBALS['shuttle_prefix'] = 'shuttle_';
...
function shuttle_customizer_setup() {
...
add_action( 'customize_register', 'shuttle_customizer_setup' );

// Enqueue customizer scripts / styles
function shuttle_customizer_adminscripts() {

	global $shuttle_theme_version;

	// Add theme stylesheets.
	wp_enqueue_style( 'shuttle-customizer', get_template_directory_uri() . '/admin/main/assets/css/shuttle-customizer.css', '', $shuttle_theme_version );
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/lib/extentions/font-awesome/css/font-awesome.min.css', '', '4.6.3' );

	// Add theme scripts.
	wp_enqueue_script( 'shuttle-customizer', get_template_directory_uri() . '/admin/main/assets/js/shuttle-customizer.js', array( 'jquery' ), $shuttle_theme_version, true );

}
add_action( 'customize_controls_enqueue_scripts', 'shuttle_customizer_adminscripts' );

// Assign custom function to fetch variable values
if ( !function_exists( 'shuttle_var' ) ) {
	function shuttle_var( $name, $key = false ) {

		global $shuttle_prefix;

		$prefix_name = $shuttle_prefix;

		$shuttle_redux_variables = get_option( 'shuttle_redux_variables' );
		$options = $shuttle_redux_variables;
